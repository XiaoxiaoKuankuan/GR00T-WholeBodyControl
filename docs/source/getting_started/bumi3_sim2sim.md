...
从 2026-09-11 起，每次 reset 在设置参考姿态后，按实际 XML 的左右
`ankle_roll_link_collision` 编译网格计算世界坐标最低点，使用
`Δz = max(0, 地面高度 + 0.0001 - 最低脚底高度)` 一次性上移真实机器人。
校正读取实际 `ground` 水平平面高度，最低脚底余量为 **0.1 mm**；不会把离地动作
往下拉，也不修改关节姿态、根朝向、初速度、源动作或红色参考影子。校正后执行
`mj_forward`，再按校正后的状态初始化历史观测。

该行为默认开启，无需新增命令参数；适用于 Robot、配对 SMPL 和无配对 SMPL 的
默认站姿初始化，也适用于 `--start-frame`、P 切换和末帧后重新开始时的 reset。
普通等待帧按 T 只开始播放，不再次校正；正常物理步和循环播放边界也不强写高度。
终端每次 reset 输出 `BUMI3_RESET_GROUND_ALIGNMENT`，包含动作名、起始帧、是否
校正、地面高度、根部上移量，以及左右脚和根部校正前后的高度，单位均为米。
本实现限定于当前水平地面和脚部 mesh；变更为其他地形或碰撞形状时会明确报错，
需要同时适配校正算法。该校正只消除初始穿地，不保证首帧静止平衡或播放质量；
正常软接触运行中仍可能出现穿入，不能以每步位置校正掩盖接触行为。
