...
def _optimizer_role_from_parameter_name(parameter_name: str) -> str:
    ...
    # Actor 初始学习率但保持固定，避免策略 KL 控制器误改附加模块的更新步长。
    return "auxiliary"
...
def build_role_aware_optimizer_groups(
    ...
    版本，因而 KL 控制、TensorBoard 日志和 checkpoint 恢复不再依赖易变的组序号。
...
def get_optimizer_learning_rates_by_role(optimizer) -> dict[str, float]:
    ...
    for role, role_rates in learning_rates.items():
        ...
        if any(
            not math.isclose(rate, first_rate, rel_tol=1.0e-12, abs_tol=0.0)
            for rate in role_rates
        ):
...
class TRLPPOTrainer(PPOTrainer):  # noqa: F405
    ...
    def _annotate_external_optimizer_groups(self) -> None:
        ...
        Actor”。这里给出明确错误，要求调用方按角色拆组，防止静默共用学习率。
        """
    ...
    def create_scheduler(self, num_training_steps: int, optimizer=None):
        ...
        Actor LR；此时不创建 scheduler，避免 constant scheduler 在同一轮末尾
        把 Actor LR 恢复到初始值，Critic LR 则始终保持独立配置值。
        """
    ...
    def _init_config(self):
        ...
        除存储 rollout/loss 所需配置外，这里会从 optimizer 实际参数组校验
        Actor/Critic 角色和 Critic 初始 LR，避免只看 YAML 声明值。
        """
        # Env related Config
    ...
    def _compute_ppo_loss(self, forward_results, mb_rollout_data):
        ...
        本函数只计算本地 KL 并写入统计缓冲，不在 loss 内即时修改学习率。
        完整 iteration 的全部 micro-batch 更新结束后，训练循环会汇总所有 GPU 的
        KL，再唯一一次决定下一轮 Actor LR；Critic LR 不受该控制器影响。
    ...
    def train(self):
        ...
        for batch_idx in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                if self._uses_adaptive_kl_schedule():
                    # 所有 epoch/minibatch/microbatch 更新已经结束；这里使用整轮、全 GPU
                    # KL 均值唯一调整一次 Actor LR，调整后的值从下一 iteration 起生效。
                ...
                # 直接读取 optimizer 参数组中的真实值；兼容标签 lr 继续表示 Actor，
                # 新标签分别展示 Actor/Critic，避免只记录过期的配置字段。
    ...
    def _migrate_legacy_optimizer_state_dict(self, legacy_state_dict: dict) -> dict:
        ...
        旧 checkpoint 没有参数名，但其两个组沿用 HuggingFace 的稳定规则：第一组为
        全部 decay 参数，第二组为全部 no-decay 参数，组内顺序等于
        ``model.named_parameters()``。本方法重建“参数名到旧 state id”的映射，复用
        Adam 动量；Actor 恢复旧 optimizer 的真实 LR，Critic 启用当前独立配置 LR。
        若模型结构或旧组形状不匹配会明确失败，不进行猜测式恢复。
        """
    ...
    def load_checkpoint(self, checkpoint_path, resume=False):  # noqa: D417
        ...
        Args:
            ...
            resume: 为真时额外恢复 optimizer、可用 scheduler、环境和 trainer
                计数器。旧两组 optimizer 会迁移到具名角色 schema；adaptive KL
                模式不恢复已禁用的 HuggingFace scheduler。
